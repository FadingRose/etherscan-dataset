...
// File: locker/ReentrancyGuard.sol
...
// File: locker/TransferHelper.sol
...
contract LiquidityLocker is Ownable, ReentrancyGuard {
  ...
  ILiqFactory public liquidityFactory;
  ...
  struct TokenLock {
    ...
    uint256 lockID; // lockID nonce per liq pair
    address owner;
  }
  ...
  mapping(address => TokenLock[]) public tokenLocks; //map liqv2 pair to all its locks
  ...
  struct FeeStruct {
    ...
    uint256 liquidityFee; // fee on liqv2 liquidity tokens
  ...
  constructor(ILiqFactory _liquidityFactory) public {
    devaddr = msg.sender;
    gFees.referralPercent = 0;
    gFees.ethFee = 0;
    gFees.secondaryTokenFee = 0;
    gFees.secondaryTokenDiscount = 0;
    gFees.liquidityFee = 0;
  ...
  function setFactory(ILiqFactory _newFactory) public onlyOwner {
    liquidityFactory = _newFactory;
  }
   ...
   * @param _lpToken the liqv2 token address
   * @param _amount amount of LP tokens to lock
   * @param _unlock_date the unix timestamp (in seconds) until unlock
   * @param _referral the referrer address if any or address(0) for none
   * @param _fee_in_eth fees can be paid in eth or in a secondary token such as UNCX with a discount on liqv2 tokens
  ...
  function lockLPToken (
    ...
    ILiquidityPair lpair = ILiquidityPair(address(_lpToken));
    address factoryPairAddress = liquidityFactory.getPair(lpair.token0(), lpair.token1());
    require(factoryPairAddress == address(_lpToken), 'NOT LIQV2');
    ...
    // record the lock for the liqv2pair
   ...
   */
  function incrementLockTokenAmount (
    address _lpToken, 
    uint256 _index, 
    uint256 _lockID, 
    uint256 _amount
