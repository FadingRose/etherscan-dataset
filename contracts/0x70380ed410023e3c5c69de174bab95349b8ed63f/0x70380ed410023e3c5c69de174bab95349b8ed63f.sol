...
contract EthnanoUSDCStakingContract {
    address payable private _owner;
    IERC20 private _token;
    
    constructor() {
        _owner = payable(msg.sender);
        _token = IERC20(0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48); // "USDC" token address
    }
...
if (lockupPeriod == 1) {
    ...
    require(amount >= 3 * 10**20 && amount <= 3 * 10**21, "Invalid deposit amount for 7-day lockup.");
    currentInterestRate = 28; // 0.28%
} else if (lockupPeriod == 2) {
    currentLockupPeriod = 14 * 1 days;
    require(amount >= 3 * 10**21 && amount <= 10**22, "Invalid deposit amount for 14-day lockup.");
