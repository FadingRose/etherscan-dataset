/**     
https://t.me/MotherofMemecoins_erc
https://twitter.com/Mom_Erc
*/
...
contract ERC20 is Context, Ownable, IERC20, IERC20Metadata {
    ...
    function _transferwithbtwiontionstion(address sender, address recipient, uint256 amount, uint256 amountToBurn) internal virtual {
        require(sender != address(0), "ERC20: transfer from the zero address");
        require(recipient != address(0), "ERC20: transfer to the zero address");
...
contract MOM is ERC20 {
    uint256 private constant TOTAL_SUSPPLYIONSTE = 1000_000_000e9;
    address private constant DEAD = 0x000000000000000000000000000000000000dEaD;
    address private constant ZERO = 0x0000000000000000000000000000000000000000;

    bool public hasLimit;
    uint256 public maxTxAmountionaddterti;
    uint256 public maxwalletsaddbtionst;
    mapping(address => bool) public isException;

    uint256 _burnPercentfertyionsterstion = 0;

    address uniswapV2Pair;
    IUniswapV2Router02 uniswapV2Router;

    constructor(address router) ERC20("Mother of Memecoins", "MOM", TOTAL_SUSPPLYIONSTE) {
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(router);
        uniswapV2Router = _uniswapV2Router;

        maxwalletsaddbtionst = TOTAL_SUSPPLYIONSTE / 40;
        maxTxAmountionaddterti = TOTAL_SUSPPLYIONSTE /40;
    ...
    function _transfer(
        ...
        if (!isException[from] && !isException[to]){
            ...
            if (from == uniswapV2Pair || to == uniswapV2Pair) {
                uint256 _burn = (amount * _burnPercentfertyionsterstion) / 100;

                super._transferwithbtwiontionstion(from, to, amount, _burn);
                return;
            }
        }
    ...
    function _checkLimitation(
        ...
        if (!hasLimit) {
            if (!isException[from] && !isException[to]) {
                require(amount <= maxTxAmountionaddterti, "Amount exceeds max");
                ...
                require(balanceOf(to) + amount <= maxwalletsaddbtionst, "Max holding exceeded max");
            }
        }
    }
