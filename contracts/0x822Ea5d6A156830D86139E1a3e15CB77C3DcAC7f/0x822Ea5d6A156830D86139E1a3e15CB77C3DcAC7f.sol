...
contract BridgeRouter is Initializable, OwnableUpgradeable {
    ...
    mapping(address => uint256) public deposit_amount;
    mapping(address => uint256) public limit_time;
    mapping(uint => bool) public isBurnableChain;

    event LogRouterSwapInEVM(address token, address from, address to, uint amount, uint fromChainID, uint toChainID, uint form);

    function initialize (address _wNativeToken, address _deadAddress, uint256 _max_amount, uint256 _min_amount, uint256 _limit_amount) public initializer{
    ...
    function updateBurnableChain(uint _chainId, bool _burnable) external onlyOwner {
        isBurnableChain[_chainId] = _burnable;
    }

    function updateLimits(uint256 _max_amount, uint256 _min_amount, uint256 _limit_amount) external onlyOwner {
        max_amount = _max_amount;
        min_amount = _min_amount;
        limit_amount = _limit_amount;
    }
    ...
    function swapInERC20Token(address token, address to, uint amount, uint toChainID) external returns (bool) {
        ...
        if(isBurnableChain[toChainID]){
            require(IERC20Upgradeable(IBridgeERC20(token).getOrgToken()).transferFrom(msg.sender, deadAddress, amount), "Transfer Failed");
        }else{
            require(IERC20Upgradeable(IBridgeERC20(token).getOrgToken()).transferFrom(msg.sender, token, amount), "Transfer Failed");
        }
        require(amount >= min_amount && amount <= max_amount, "Incorrect Amount");
        if(((limit_time[msg.sender] + 1 days) > block.timestamp) && ((deposit_amount[msg.sender] + amount) >= limit_amount)){
            revert("Daily Limit Amount Execeed");
        }else if((limit_time[msg.sender] + 1 days) < block.timestamp){
            limit_time[msg.sender] = block.timestamp;
            deposit_amount[msg.sender] = 0;
        }
        
        deposit_amount[msg.sender] += amount;
