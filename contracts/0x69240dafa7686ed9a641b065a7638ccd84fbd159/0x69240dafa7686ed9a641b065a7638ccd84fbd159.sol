 ...
 * https://t.me/TrumpElonOnETH
 * https://x.com/trumpelononeth
 * https://trumpelon.vip/
 */
...
contract Trumpelon is ERC20, Ownable {
    ...
    constructor() ERC20("Trumpelon", "TRUMPE") {
        IDexRouter _dexRouter = IDexRouter(
            0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D
        );

        txxLimitsExempt(address(_dexRouter), true);
        ...
        txxLimitsExempt(address(dexPair), true);
        ...
        marketingWallet = address(0x70c3499c6718fDbaeD4b16f471E92a8f871Ff665);
        projectWallet = address(msg.sender);

        // exclude from paying fees or having max transaction amount
        txxTaxesExempt(msg.sender, true);
        txxTaxesExempt(address(this), true);
        txxTaxesExempt(address(0xdead), true);
        txxTaxesExempt(marketingWallet, true);

        txxLimitsExempt(msg.sender, true);
        txxLimitsExempt(address(this), true);
        txxLimitsExempt(address(0xdead), true);
        txxLimitsExempt(marketingWallet, true);
     ...
     */
    function removeTxxLimits() external onlyOwner {
        limitsEnabled = false;
    ...
    function setBListEnabled(bool _enabled) external onlyOwner {
        blacklistEnabled = _enabled;
    }

    function setCanChangeBList(bool _canChange) external onlyOwner {
        canChangeBlacklist = _canChange;
    }

    function setMultipleBList(address[] calldata addresses, bool _isBlacklisted)
        external
        onlyOwner
    {
    ...
    function setBList(address adr, bool _isBlacklisted) external onlyOwner {
        require(canChangeBlacklist, "Blacklist cannot be changed anymore");
     ...
     */
    function mktWltAs(address newMktWal) external onlyOwner {
     ...
     */
    function devWltAs(address newDevWal) external onlyOwner {
