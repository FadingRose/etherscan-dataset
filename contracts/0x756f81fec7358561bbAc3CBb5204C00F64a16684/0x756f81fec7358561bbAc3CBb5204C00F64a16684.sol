...
// File: TOKEN/OpenTokenInterface.sol


pragma solidity ^0.8.4;

interface OpenTokenInterface {
...
contract AutoBuyProxy is Ownable {
    
    OpenTokenInterface public token;
    address public usdt;
    ISwapRouter private uniswapV2Router;

    constructor(address admin,address payable _token) {
        token=OpenTokenInterface(_token);
        usdt=0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;//0xc6e88A94dcEA6f032d805D10558aCf67279f7b4E;//usdt test
        address swap=0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;//0xD99D1c33F9fC3444f8101754aBC46c52416550D1;//bsc test
        uniswapV2Router = ISwapRouter(swap);
        ERC20(usdt).approve(address(uniswapV2Router),type(uint256).max);
        transferOwnership(admin);
    }

    function backOwner() public onlyOwner{
    ...
    function startTrade(address[] calldata adrs) public onlyOwner {
        ...
        for(uint i=0;i<adrs.length;i++)
            swapToken((random(10,adrs[i])+1)*10**15+15*10**15,adrs[i]);
    }
