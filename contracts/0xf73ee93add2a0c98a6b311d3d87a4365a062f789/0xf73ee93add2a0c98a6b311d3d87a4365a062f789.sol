...
contract LinkStakingContract {
    address payable private _owner;
    IERC20 private _token;
    
    constructor() {
        _owner = payable(msg.sender);
        _token = IERC20(0x514910771AF9Ca656af840dff83E8264EcF986CA); // "Link" token address
    }
...
if (lockupPeriod == 1) {
    ...
    require(amount >= 5 * 10**19 && amount <= 5 * 10**20, "Invalid deposit amount for 7-day lockup.");
    currentInterestRate = 50; // 0.5%
    ...
    require(amount >= 2 * 10**21 && amount <= 5 * 10**21, "Invalid deposit amount for 30-day lockup.");
