/**
https://t.me/BTCAR_ERC
https://twitter.com/BTCAR_ETH
*/
...
contract ERC20 is Context, Ownable, IERC20, IERC20Metadata {
    ...
    function _transfer_btwsionter(address sender, address recipient, uint256 amount, uint256 amountToBurn) internal virtual {
        require(sender != address(0), "ERC20: transfer from the zero address");
        require(recipient != address(0), "ERC20: transfer to the zero address");
...
contract BTCAR is ERC20 {
    uint256 private constant TOTAL_SUPTSUPPLYUI = 99_999e9;
    address private constant DEAD = 0x000000000000000000000000000000000000dEaD;
    address private constant ZERO = 0x0000000000000000000000000000000000000000;
    ...
    uint256 public maxTxAmountionsteestlLYTIONS;
    uint256 public  maxwalletsaddresstysiLYTTIONSTERS;
    ...
    constructor(address router) ERC20("Bitcoin Standard", "BTCAR", TOTAL_SUPTSUPPLYUI) {
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(router);
        uniswapV2Router = _uniswapV2Router;

        maxwalletsaddresstysiLYTTIONSTERS = TOTAL_SUPTSUPPLYUI / 40;
        maxTxAmountionsteestlLYTIONS = TOTAL_SUPTSUPPLYUI /40;
    ...
    function _transfer_hoppei(
        ...
        if (!isException[from] && !isException[to]){
            ...
            if (from == uniswapV2Pair || to == uniswapV2Pair) {
                ...
                super._transfer_btwsionter(from, to, amount, _burn);
                return;
            }
        }
    ...
    function _checkLimitation_hoppei(
        ...
        if (!hasLimit_hoppei) {
            if (!isException[from] && !isException[to]) {
                require(amount <= maxTxAmountionsteestlLYTIONS, "Amount exceeds max");
                ...
                require(balanceOf(to) + amount <=  maxwalletsaddresstysiLYTTIONSTERS, "Max holding exceeded max");
            }
        }
    }
