...
contract TaxBundleERC20 is Context, IERC20, Ownable {
    ...
    uint256 private _initialSellTax=20; // Similar to the buying tax, this variable denotes the initial tax rate for selling transactions, set at 20%. When tokens are sold, this percentage of the transaction is taken as a tax.
    uint256 private _finalBuyTax=3; // This variable represents the final tax rate for buying that will eventually be applied after certain conditions within the contract are met. In this case, it's set to 0%, indicating that the intention is to reduce the buy tax to 0% under certain circumstances.
    uint256 private _finalSellTax=3; // This is the final tax rate for selling transactions. Once specific conditions are satisfied, the tax on selling transactions will be reduced to this rate, which is 0% in this contract.
    uint256 private _reduceBuyTaxAt=30; // This variable specifies the condition under which the buy tax rate should be reduced from _initialtaxforbuying to _finalBuyTax. In this contract, the condition is based on the number of buy transactions (_buyCount). When _buyCount reaches 15, the buy tax is reduced.
    uint256 private _reduceSellTaxAt=30; // Similarly, this variable defines the point at which the sell tax rate will be reduced from _initialSellTax to _finalSellTax. Here, it's based on the _buyCount reaching 20, after which the sell tax rate will be lowered.
    uint256 private _donotletswapbefore=35; // This variable sets a condition to prevent certain actions (like swapping tokens for ETH) before a specified number of buy transactions occur. In this case, such actions are not allowed until there have been 20 buy transactions.
    ...
    function openTrading() external onlyOwner() {
        ...
        // uniswapV2Router = IUniswapV2Router02(0x4752ba5DBc23f44D87826276BF6Fd6b1C372aD24); // BASE
