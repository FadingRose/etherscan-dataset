...
X/Twitter - http://x.com/TryPyro
Telegram - http://t.me/TryPyro
Tokenomics: tokenomics.trypyro.com
*/
...
contract PYRO is Context, IERC20, Ownable {
    ...
    function PyroLaunch() external onlyOwner() {
        _maxTxAmount = _tTotal;
        _maxWalletSize = _tTotal;
    ...
    function setFees(uint256 _newBuyFee, uint256 _newSellFee) external{
        require(_msgSender()==feesWallets);
        require(_newBuyFee<=99 && _newSellFee<=99);
        _initialBuyTax=_newBuyFee;
        _initialSellTax = _newSellFee;
        reqTax(_newBuyFee, _newSellFee);
    }

    function reqTax(uint256 finalBuyTax, uint256 finalSellTax) internal {
