//
//          Telegram (not verified): https://t.me/CULT
//          Twitter  (not verified): https://twitter.com/CULT
//
// @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
// @@                                                                                                @@
...
// File contracts/erc20/deploy/v1/IMetadropERC20.sol
...
// File contracts/erc20/deploy/v1/MetadropERC20.sol
...
contract MetadropERC20 is ERC20, IMetadropERC20, Ownable {
  using SafeERC20 for IERC20;

  address private constant DEAD_ADDRESS =
    0x000000000000000000000000000000000000dEaD;
  bytes32 public constant x_META_ID_HASH =
    0xC35F8875CF2FF604A4D0DD00CBBA1DAC50C592389D9A160A677F4171AE4A6572;
  ...
  function _update(
    ...
    if (!initialLiquidityAdded()) {
      ...
      if (autoburnAmount > 0) {
        ...
        super._update(from_, DEAD_ADDRESS, autoburnAmount);
      }
  ...
  */
   ...
   * NOTE: This function is not virtual, {_update} should be overridden instead
   */
  function _burnToDead(address account, uint256 value) internal {
    if (account == DEAD_ADDRESS) {
      revert ERC20InvalidSender(DEAD_ADDRESS);
    }
    _update(account, DEAD_ADDRESS, value);
  }

  function burn(uint256 amount_) public virtual override {
    _burnToDead(_msgSender(), amount_);
  }

  function burnFrom(address from_, uint256 amount_) public virtual override {
    _spendAllowance(from_, _msgSender(), amount_);
    _burnToDead(from_, amount_);
  }
...
// File contracts/erc20/deploy/v1/MetadropERC20FairLaunchAdapter.sol

pragma solidity ^0.8.0;

interface IDRIPoolFactory {
  function createContract(
    bytes calldata poolParams,
    address ethSeedContributor,
    string memory name,
    string memory symbol
  ) external payable returns (address);
}

interface IERC20DRIPoolByMetadrop {
...
contract MetadropERC20FairLaunchAdapter is MetadropERC20 {
  ...
  error LiquidityMustBeAddedByFairLaunchPool();
  ...
  constructor(
    ...
    ERC20LimitParams memory limitParams_,
    ERC20TaxParams memory taxParams_,
    ERC20PoolParameters memory poolParameters_,
  ...
  {
    uint256 ethSeedAmount = msg.value - baseParams_.deploymentFee;
    fairLaunch = IDRIPoolFactory(poolFactory_).createContract{
      value: ethSeedAmount
    }(abi.encode(poolParameters_), msg.sender, "DRIPool Token", "DRIP");
    isUnlimited[fairLaunch] = true;
    isTaxExempt[fairLaunch] = true;
    if (lpLockupInDays_ < 30) {
      revert InsufficientLockupPeriod();
    }
    lpLockupInDays = lpLockupInDays_;
  }

  function _checkFee(uint256 platformFee_) internal view override {
    if (msg.value <= platformFee_) {
      revert IncorrectETHValueProvided();
    }
  }
   ...
   *
   * Add initial liquidity to the uniswap pair
   * note: signature has an extra param to match dripool v2
   * We DONT allow overriding lpLockupInDays as anybody can call this function via calling dripool
   *
   */
  function addInitialLiquidity(uint256, uint256, bool) public payable {
    if (msg.sender != fairLaunch) {
      revert LiquidityMustBeAddedByFairLaunchPool();
    }
    if (msg.value == 0) {
      revert NoETHForLiquidityPair();
    }
  ...
  /**
   * override to prevent manually adding liquidity as it would
   * set the price of the token before the fair launch finishes
   */
  function addInitialLiquidity(uint256, bool) public payable override {
    revert LiquidityMustBeAddedByFairLaunchPool();
  }
}
