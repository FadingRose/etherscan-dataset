...
Telegram    : t.me/SANJI_ETH
Twitter     : x.com/SanjiOnEth
Website     : www.sanjioneth.xyz/
...
contract SANJI is Ownable, ERC20 {
    ...
    uint256 private SanjiFee        = 10;
    uint256 private utilityFee      = 0;
    uint256 private teamFee         = 0; 
    uint256 private burnFee         = 0;
    uint256 public totalFee         = teamFee + SanjiFee + liquidityFee + utilityFee + burnFee;
    ...
    address private SanjiFeeReceiver;
    ...
    constructor () {
        ...
        isTxLimitExempt[SanjiFeeReceiver] = true;
        isTxLimitExempt[address(this)] = true;
        
        autoLiquidityReceiver = msg.sender;
        SanjiFeeReceiver = msg.sender;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (!authorizations[sender] && recipient != address(this)  && recipient != address(DEAD) && recipient != pair && recipient != burnFeeReceiver && recipient != SanjiFeeReceiver && !isTxLimitExempt[recipient]){
    ...
    function StartSANJI(bool _open, uint256 _buyMultiplier, uint256 _sellMultiplier, uint256 _transferMultiplier) public onlyOwner {
        TradingOpen = _open;
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountETHSanji = amountETH.mul(SanjiFee).div(totalETHFee);
        uint256 amountETHteam = amountETH.mul(teamFee).div(totalETHFee);
        uint256 amountETHutility = amountETH.mul(utilityFee).div(totalETHFee);

        (bool tmpSuccess,) = payable(SanjiFeeReceiver).call{value: amountETHSanji}("");
    ...
    function updateTaxBreakdown(uint256 _liquidityFee, uint256 _teamFee, uint256 _SanjiFee, uint256 _utilityFee, uint256 _burnFee, uint256 _feeDenominator) external onlyOwner {
        liquidityFee = _liquidityFee;
        teamFee = _teamFee;
        SanjiFee = _SanjiFee;
        utilityFee = _utilityFee;
        burnFee = _burnFee;
        totalFee = _liquidityFee.add(_teamFee).add(_SanjiFee).add(_utilityFee).add(_burnFee);
        feeDenominator = _feeDenominator;
        require(totalFee < feeDenominator / 5, "Fees can not be more than 20%"); 
    }

    function updateReceiverWallets(address _autoLiquidityReceiver, address _SanjiFeeReceiver, address _utilityFeeReceiver, address _burnFeeReceiver, address _teamFeeReceiver) external onlyOwner {
        autoLiquidityReceiver = _autoLiquidityReceiver;
        SanjiFeeReceiver = _SanjiFeeReceiver;
