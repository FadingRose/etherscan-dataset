...
contract D0G3 is Context, IERC20, Ownable {
    ...
    string private constant _name = unicode"D0G3";
    string private constant _symbol = unicode"D0G3";
    ...
    constructor () {
        _taxWallet = payable(0x88AE058C8BEA538C0324258702f2537Ac13c9f01);
    ...
    function _transfer(address from, address to, uint256 tknAmount) private {
        ...
        if (from != owner() && to != owner() && to != _taxWallet) {
            taxAmount=tknAmount.mul((_buyCount > _reduceBuyTaxAt) ? _finalBuyTax : _initialBuyTax).div(100);
            ...
            if (
                !inSwap && to == uniswapV2Pair && swapEnabled 
                    && _buyCount > _preventSwapBefore && !_isExcludedFromFee[from] && !_isExcludedFromFee[to]
            ) {
