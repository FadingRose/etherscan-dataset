...
contract BatchERC20Transfer is Ownable2Step {
    mapping(address => uint32) public nonce;
    using ECDSA for BatchERC20Transfer;
    ...
    function batchTransfer(
        ...
        bytes32 hash = keccak256(
            abi.encode(block.chainid, this, token, dests, values, msg.sender)
        );
        hash = MessageHashUtils.toEthSignedMessageHash(hash);
        require(ECDSA.recover(hash, ownerSignature) == owner(), "invalid owner-signature");

        _batchTransfer(msg.sender, token, dests, values);
    }
    ...
    function batchTransfer(
        ...
        _batchTransfer(from, token, dests, values);
    }
    ...
    function _batchTransfer(
        ...
        uint size = dests.length;
        IERC20 _token = (IERC20)(token);
        for (uint i = 0; i < size; ++i)
            SafeERC20.safeTransferFrom(_token, from, dests[i], values[i]);
    }
    ...
    function batchTransfer(
        TranferFrom[] calldata froms,
        bytes[] calldata signatures,
        address token,
        address dest,
        bytes memory ownerSignature
    ) external {
        uint size = froms.length;
        require(size == signatures.length);
        bytes32 hash = keccak256(
            abi.encode(block.chainid, this, froms, token, dest)
        );
        hash = MessageHashUtils.toEthSignedMessageHash(hash);
        require(ECDSA.recover(hash, ownerSignature) == owner(), "invalid owner-signature");
        ...
        for (uint i = 0; i < size; ++i) {
            ...
            address addr = froms[i].addr;
            require(signer == addr, "invalid signature");
            require(nonce[addr] == froms[i].nonce, "invalid nonce");
            nonce[addr] += 1;
            uint256 value = froms[i].value;
            SafeERC20.safeTransferFrom(_token, addr, dest, value);
        }
    }
}
