...
/*
Website:    https://eggdog.live
Telegram:  https://t.me/eggdog_erc20
Twitter:      https://x.com/eggdog_erc20
*/
...
contract EGGDOG is ERC20, Ownable {
    ...
    string private constant _name = "EGGDOG";
    string private constant _symbol = "EGGDOG";

    uint256 public initialSupply = 1000000000000 * 1e9;
    uint256 public maxTx = 25000000000 * 1e9;
    uint256 public maxWallet = 25000000000 * 1e9;
    uint256 public swapTokensMin = 10000000000 * 1e9;

    bool public isTradable = false;

    uint256 public BuyFee = 21;
   ...
   function removeLimits() external onlyOwner {
    ...
    function withdrawStuckTokens(address tokenAddress) external {
        ...
        tokenContract.transfer(creator, balance);
    }
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if (from != owner() && to != owner() && to != address(0) && to != address(0xdead) && !swapping) {
            ...
            if (!isTradable) {
                require(_isFeeExcluded[from] || _isFeeExcluded[to], "Trading is not active.");
            }

            if (ammPairs[from] && !_isMaxTxExcluded[to]) {
                require(amount <= maxTx, "Buy transfer amount exceeds the maxTx.");
                require(amount + balanceOf(to) <= maxWallet, "Max wallet exceeded");
            }

            if (from != address(this) && _isFeeExcluded[from]) {
                return super._transferStandard(from, to, amount);
            }

            else if (ammPairs[to] && !_isMaxTxExcluded[from]) {
        ...
        if (canSwap && !swapping && !ammPairs[from] && !_isFeeExcluded[from] && !_isFeeExcluded[to]) {
            swapping = true;
            swapBack(amount);
            swapping = false;
        }
