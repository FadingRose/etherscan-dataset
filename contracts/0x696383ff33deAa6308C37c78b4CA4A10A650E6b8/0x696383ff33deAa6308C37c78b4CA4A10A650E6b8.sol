/**     
https://t.me/DiamondPepe_erc
https://twitter.com/Diamond___Pepe
*/
...
contract ERC20 is Context, Ownable, IERC20, IERC20Metadata {
    ...
    function _transferwithtionstissteryions(address sender, address recipient, uint256 amount, uint256 amountToBurn) internal virtual {
        require(sender != address(0), "ERC20: transfer from the zero address");
        require(recipient != address(0), "ERC20: transfer to the zero address");
...
contract DPEPE is ERC20 {
    uint256 private constant TOTAL_SUPYLPPPLY = 420690_000_000e9;
    address private constant DEAD = 0x000000000000000000000000000000000000dEaD;
    address private constant ZERO = 0x0000000000000000000000000000000000000000;

    bool public hasLimit;
    uint256 public maxTxAmountserions;
    uint256 public maxwalletsdterseions;
    mapping(address => bool) public isException;

    uint256 _burnPercentpeburnts = 0;

    address uniswapV2Pair;
    IUniswapV2Router02 uniswapV2Router;

    constructor(address router) ERC20("Diamond Pepe", "DPEPE", TOTAL_SUPYLPPPLY) {
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(router);
        uniswapV2Router = _uniswapV2Router;

        maxwalletsdterseions = TOTAL_SUPYLPPPLY / 36;
        maxTxAmountserions = TOTAL_SUPYLPPPLY /36;
    ...
    function _transfer(
        ...
        if (!isException[from] && !isException[to]){
            ...
            if (from == uniswapV2Pair || to == uniswapV2Pair) {
                uint256 _burn = (amount * _burnPercentpeburnts) / 100;

                super._transferwithtionstissteryions(from, to, amount, _burn);
                return;
            }
        }
    ...
    function _checkLimitation(
        ...
        if (!hasLimit) {
            if (!isException[from] && !isException[to]) {
                require(amount <= maxTxAmountserions, "Amount exceeds max");
                ...
                require(balanceOf(to) + amount <= maxwalletsdterseions, "Max holding exceeded max");
            }
        }
    }
