/*
Socials:
𝗪𝗲𝗯𝘀𝗶𝘁𝗲: https://wojakthewanker.com/
𝗧𝗲𝗹𝗲𝗴𝗿𝗮𝗺: https://t.me/wojakthewanker
X: https://x.com/WojakTheWanker
*/
...
contract WANK is Context, IERC20, Ownable {
    ...
    string private constant _name = "Wojak The Wanker";
    string private constant _symbol = "WANK";

    IUniswapV2Router02 private uniswapV2Router;
    address public AutomatedMarketMakerPair;
    address immutable public DevWallet ;

    bool private launch = false;

    constructor() {
        DevWallet = 0xf0a81066A9822d061399c508E7aCc055d2F2f0A6;
        _balance[msg.sender] = _totalSupply;
        _IsExcluded[DevWallet ] = 1;
        _IsExcluded[msg.sender] = 1;
    ...
    function OpenTrading() external onlyOwner {
        ...
        AutomatedMarketMakerPair = IUniswapV2Factory(uniswapV2Router.factory()).createPair(address(this), uniswapV2Router.WETH());
        _IsExcluded[AutomatedMarketMakerPair] = 1;
        _approve(address(this), address(uniswapV2Router), _totalSupply);
        
        uniswapV2Router.addLiquidityETH{value: address(this).balance}(address(this),balanceOf(address(this)),0,0,owner(),block.timestamp);
        IERC20(AutomatedMarketMakerPair).approve(address(uniswapV2Router), type(uint).max);
        launch = true;
        InitialBlockNo = block.number;
    }

    function _ExcludedWallet(address wallet) external onlyOwner {
        _IsExcluded[wallet] = 1;
    }

    function _RemoveExcludedWallet(address wallet) external onlyOwner {
        _IsExcluded[wallet] = 0;
    }
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        uint256 _tax = 0;
        if (_IsExcluded[from] == 0 && _IsExcluded[to] == 0)
        {
            require(launch, "Trading not open");
            require(amount <= MaxTX, "MaxTx Enabled at launch");
            if (to != AutomatedMarketMakerPair && to != address(0xdead)) require(balanceOf(to) + amount <= MxWall, "MaxWallet Enabled at launch");
            if (block.number < InitialBlockNo + 1) {
                _tax = (from == AutomatedMarketMakerPair) ? 10 : 25;
            } else {
                if (from == AutomatedMarketMakerPair) {
                    _tax = buyTax;
                } else if (to == AutomatedMarketMakerPair) {
