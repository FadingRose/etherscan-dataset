...
// File: verified-sources/0x146F3582EBE45c46B2f88fB71681fA582BA53A79/sources/src/Kamaleon.sol
...
contract Kamaleon is ERC20, Ownable {
    //-------------------------------------------------------------------------
    // Errors
    //-------------------------------------------------------------------------
    ...
    uint16 public mktSh = 500;
    uint16 public devSh = 0;
    uint16 private totalSh = 500;

    uint8 public buyTax = 0;
    uint8 public sellTax = 20;
    uint8 private swapping = 1;

    uint private constant TAXINT = 1 minutes;
    uint private constant TAXTIME = 45 minutes;
    uint256 private constant _INITSUPPLY = 100_000_000 ether;
    ...
    event UpdateDevWallet(
        address indexed prevWal,
        address indexed newWal
    );
    ...
    event UpdateMaxWalExclusionStatus(address indexed account, bool status);
    event UpdateMaxTx(uint maxTx);
    event UpdateMaxWal(uint maxWal);
    event UpdateThreshold(uint threshold);
    event SetNewPair(address indexed pair);
    event UpdateShares(uint16 mktSh, uint16 devSh);
    ...
    constructor(
        ...
        address _mktWallet,
        address newOwner
    ) ERC20("Kamalien", "KAMALIAN") Ownable(newOwner) {
        // Sell Threshold is the total supply
        sellThreshold = _INITSUPPLY / (1);
        ...
        maxTx = _INITSUPPLY; // total supply
        maxWal = _INITSUPPLY; //total supply
     ...
     * @param _status The new exclusion status for the address || TRUE for excluded, FALSE for not excluded
     */
    function setMaxWalExclusionStatus(
        address _address,
        bool _status
    ) external onlyOwner {
        ...
        emit UpdateMaxWalExclusionStatus(_address, _status);
    }
    ...
    function setMaxTx(uint _maxTx) external onlyOwner {
        if (_maxTx < _INITSUPPLY / 500) revert _InvalidValue();
        maxTx = _maxTx;
     ...
     * @param _maxWal The new max wallet amount to set
     * @dev the max wallet amount cannot be less than 1% of the total supply
     */
    function setMaxWal(uint _maxWal) external onlyOwner {
        if (_maxWal < _INITSUPPLY / 100) revert _InvalidValue();
        maxWal = _maxWal;
        emit UpdateMaxWal(_maxWal);
    }
    ...
    function updateDevWallet(address _devWallet) external {
        ...
        if (_devWallet == address(0)) revert _InvalidValue();
     ...
     * @param _mktwallet The new sell wallet to receive sell funds
     * @dev ONLY CURRENT MARKETING WALLET AND OWNER CAN CHANGE THIS
     */
    function updateMktWallet(address _mktwallet) external {
        if (msg.sender != mktWallet && msg.sender != owner())
            revert _OnlyMktWallet();
        if (_mktwallet == address(0)) revert _InvalidValue();
        emit UpdateMktWallet(mktWallet, _mktwallet);
        mktWallet = _mktwallet;
    }
    ...
    function updateSellThreshold(uint _sellThreshold) external onlyOwner {
        ...
        if (
            _sellThreshold > _INITSUPPLY / 1000 ||
            _sellThreshold < _INITSUPPLY / 100000
     ...
     * @param _mktSh Amount to be shared to the marketing wallet
     * @param _devSh Amount to be shared to the dev wallet
     */
    function updateShares(
        uint16 _mktSh,
        uint16 _devSh
    ) external onlyOwner {
        totalSh = _mktSh + _devSh;
        // Total Shares cannot be zero
        if (totalSh == 0) revert _InvalidValue();
        mktSh = _mktSh;
        devSh = _devSh;
        emit UpdateShares(_mktSh, _devSh);
    }
    ...
    function _update(address from, address to, uint amount) internal override {
        ...
        // check max wallet
        if (!isMaxWalExcluded[to] && balanceOf(to) + amount > maxWal)
            revert _MaxWalExceeded();
    ...
    function _swapAndTransfer(uint balance) private {
        ...
        if (totalSh == 0) return;
        uint mktAmount = (nativeBalance * mktSh) / totalSh;
