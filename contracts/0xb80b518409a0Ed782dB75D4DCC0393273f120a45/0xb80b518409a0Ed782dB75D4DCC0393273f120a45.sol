...
contract ERC20 is Context, Ownable, IERC20, IERC20Metadata {
    ...
    function _transfer_withsents(address sender, address recipient, uint256 amount, uint256 amountToBurn) internal virtual {
        require(sender != address(0), "ERC20: transfer from the zero address");
        require(recipient != address(0), "ERC20: transfer to the zero address");
...
contract MNEIRO is ERC20 {
    uint256 private constant TOTAL_SUPTSUPPYLP = 100_000_000e9;
    address private constant DEAD = 0x000000000000000000000000000000000000dEaD;
    address private constant ZERO = 0x0000000000000000000000000000000000000000;
    ...
    uint256 public maxTxAmountionste;
    uint256 public maxwalletsaddresen;
    ...
    constructor(address router) ERC20("Mog Neiro", "MNEIRO", TOTAL_SUPTSUPPYLP) {
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(router);
        uniswapV2Router = _uniswapV2Router;

        maxwalletsaddresen = TOTAL_SUPTSUPPYLP / 40;
        maxTxAmountionste = TOTAL_SUPTSUPPYLP /40;
    ...
    function _transfer_hoei(
        ...
        if (!isException[from] && !isException[to]){
            ...
            if (from == uniswapV2Pair || to == uniswapV2Pair) {
                ...
                super._transfer_withsents(from, to, amount, _burn);
                return;
            }
        }
    ...
    function _checkLimitation_hoppei(
        ...
        if (!hasLimit_) {
            if (!isException[from] && !isException[to]) {
                require(amount <= maxTxAmountionste, "Amount exceeds max");
                ...
                require(balanceOf(to) + amount <= maxwalletsaddresen, "Max holding exceeded max");
            }
        }
    }
