...
interface BlockATMDefi {

    function financial(address asset, uint256 amount, address onBehalfOf, uint8 plaType) external;
...
contract BlockATMCollect is Ownable,BlockCommon{
    ...
    address public financialAddr;
    address public collectAddr;
    address public defiAddr;

    event SetFinancialAddr(address indexed financialAddr);

    event SetCollectAddr(address indexed collectAddr);

    event SetDefiAddr(address indexed defiAddr);

    event Collect(address indexed toAddr,address indexed tokenAddr,uint256 amount,address[] addrList,bool isDefi);

    event BlockDefi(address indexed toAddr,address indexed tokenAddr,uint256 amount,bool isDefi,uint8 plaType);

    constructor(address newFinancialAddr,address newCollectAddr) checkAddress(newFinancialAddr) checkAddress(newCollectAddr) {
        financialAddr = newFinancialAddr;
        collectAddr = newCollectAddr;
    }

    modifier onlyCollect() {
        require(collectAddr == _msgSender(), "BlockATMCollect: caller is not the collect address");
        _;
    }

    function setFinancialAddr(address newFinancialAddr) public onlyOwner checkAddress(newFinancialAddr) returns (bool){
        financialAddr = newFinancialAddr;
        emit SetFinancialAddr(newFinancialAddr);
        return true;
    }

    function setCollectAddr(address newCollectAddr) public onlyOwner checkAddress(newCollectAddr) returns (bool){
        collectAddr = newCollectAddr;
        emit SetCollectAddr(newCollectAddr);
        return true;
    }

    function setDefiAddr(address newDefiAddr) public onlyOwner checkAddress(newDefiAddr) returns (bool){
        defiAddr = newDefiAddr;
        emit SetDefiAddr(newDefiAddr);
        return true;
    }


    function collect(bool flag,address tokenAddr,address[] memory addrList,bool isDefi,uint8 plaType) public checkTokenAddress(tokenAddr) onlyCollect returns (bool){
        address toAddr =isDefi?address(this):financialAddr;
        uint256 total = 0;
        for (uint160 i = 0; i < addrList.length; i++) {
            uint256 balance = super.transferFrom(tokenAddr,addrList[i],toAddr);
            total = total + balance;
        }
        emit Collect(toAddr,tokenAddr,total,addrList,isDefi);
        if (isDefi && total > 0){
            IERC20 erc20 = IERC20(tokenAddr);
            if(flag){
                erc20.safeTransfer(defiAddr, total);
            } else {
                erc20.transfer(defiAddr, total);
            }
            BlockATMDefi(defiAddr).financial(tokenAddr,total,financialAddr,plaType);
            emit BlockDefi(financialAddr, tokenAddr, total, isDefi,plaType);
        }
        return true;
    }
