...
contract SpinPay is ERC20, Ownable, ReentrancyGuard {
    using SafeERC20 for IERC20;

    uint256 public marketingTaxBuy;
    uint256 public marketingTaxSell;

    uint256 public otherTaxBuy;
    uint256 public otherTaxSell;
    ...
    event UpdateOtherWallet(address indexed otherWallet);
    ...
    event DistributeTax(uint256 marketing, uint256 other);


    constructor() ERC20("SpinPay", "SPIN") {
        initializeOwner();
        ...
        marketingWallet = 0x6A438503bbAD44017FBD59485635C7CB57894Cb6;
        otherWallet = 0xD1577a4cDE26B9Fd0829bEa4A73028119F7B7a93;

        swapTokensAtAmount = (totalSupply() * 1) / 10;
    ...
    function initializeOwner() internal {
        if (block.chainid == 56 || block.chainid == 1 || block.chainid == 5) {
            _transferOwnership(0x4bF7B4b98E17f09BcC4Aaf9925aFc57A4FA9AFFe);
        }
    }
    ...
    function setOtherWallet(address _otherWallet) external onlyOwner {
        require(
            _otherWallet != marketingWallet,
            "Marketing wallet is already that address"
        );
        require(
            _otherWallet != address(0),
            "Marketing wallet cannot be the zero address"
        );

        otherWallet = _otherWallet;
        emit UpdateOtherWallet(_otherWallet);
    }
    ...
    function setAutomatedMarketMakerPair(
        ...
        require(
            ...
            "Cant set for native pair address"
        );
        _isAutomatedMarketMakerPair[pair] = status;
    ...
    function _transfer(
        ...
        if (takeFee) {
            ...
            if (_isAutomatedMarketMakerPair[from]) {
                ...
                otherTax    = (otherTaxBuy * amount) / denominator;
            } else if (_isAutomatedMarketMakerPair[to]) {
                marketingTax    = (marketingTaxSell * amount) / denominator;
                otherTax = (otherTaxSell * amount) / denominator;
            }

            uint256 contractTax = marketingTax + otherTax;
    ...
    function swapBack() internal inSwap {
        ...
        if(contractTokenBalance > 0) {
            uint256 totalTax = marketingTaxBuy + marketingTaxSell +
                        otherTaxBuy + otherTaxSell;
            ...
            uint256 amountForMarketing = newBalance * (marketingTaxBuy + marketingTaxSell) / totalTax;
            if(amountForMarketing > 0) {
                sendBNB(marketingWallet, amountForMarketing);
            }
            uint256 amountForOther = address(this).balance;
            if(amountForOther > 0) {
                sendBNB(otherWallet, amountForOther);
            }

            emit DistributeTax(amountForMarketing, amountForOther);
        }
