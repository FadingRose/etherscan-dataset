...
contract BridgeERC20 is ERC20, Ownable, MinterRole {
    ...
    constructor(string memory _name, string memory _symbol, uint8 _decimal, address _orgToken, address _feeReceiver, uint256 _bridgeFee) ERC20(_name, _symbol) {
        orgToken = _orgToken;
        decimal = _decimal;
    ...
    function updateFeeConfig(address _feeReceiver, uint256 _bridgeFee) external onlyOwner {
        feeReceiver = _feeReceiver;
        bridgeFee = _bridgeFee;
    }
        ...
        if(orgToken != address(0)) {
            uint256 feeAmount = amount * bridgeFee / 10000;
                if(form == 0) {
                    ...
                    IERC20(orgToken).transfer(to, (amount - feeAmount));
                    IERC20(orgToken).transfer(feeReceiver, (feeAmount));
