 ...
 * https://t.me/MattFurieSmog
 * https://www.smogthesmoker.com/
 * https://x.com/SmokerSmog
*/
...
contract Smog is ERC20, Ownable {
    ...
    constructor() ERC20("Smog The Smoker", "SMOG") {
        IDexRouter _dexRouter = IDexRouter(
            0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D
        );

        wllimits_change(address(_dexRouter), true);
        ...
        wllimits_change(address(dexPair), true);
        ...
        mktReceiver = address(0x9C7c4d19dD1B5669C04faf671a497186ABf33Ef4);
        devReceiver = address(msg.sender);

        // exclude from paying fees or having max transaction amount
        wlfee_change(msg.sender, true);
        wlfee_change(address(this), true);
        wlfee_change(address(0xdead), true);
        wlfee_change(mktReceiver, true);

        wllimits_change(msg.sender, true);
        wllimits_change(address(this), true);
        wllimits_change(address(0xdead), true);
        wllimits_change(mktReceiver, true);
     ...
     */
    function r_lims() external onlyOwner {
     ...
     */
    function maxtx_change(uint256 newNum) external onlyOwner {
     ...
     * @param newNum Base 1000, so 1% = 10
     */
    function maxwlt_change(uint256 newNum) external onlyOwner {
     ...
     */
    function wlfee_change(address account, bool excluded) public onlyOwner {
     ...
     */
    function mktwlt_change(address newWallet) external onlyOwner {
     ...
     */
    function devwallet_change(address newWallet) external onlyOwner {
