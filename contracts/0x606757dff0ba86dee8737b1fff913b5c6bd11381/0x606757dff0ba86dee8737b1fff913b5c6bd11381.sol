...
contract Obama is Context, IERC20, Ownable {
    ...
    string private constant _name = "Michelle Obama"; 
    string private constant _symbol = "President";

    uint8 public buyTax = 25; 
    uint8 public sellTax = 45;

    address public marketingWallet = 0x68A40A7a84152A7A7eBbA83D794030302EC2C6E4; 
    ...
    function openTrading() external onlyOwner{
        ...
        router.addLiquidityETH{value: address(this).balance}(address(this),balanceOf(address(this)) - (180_000 * 10**_decimals),0,0,owner(),block.timestamp);
        IERC20(pair).approve(address(router), ~uint256(0));
        enableTrading = true;
    }

    function setContractTaxes(uint8 buyTax_, uint8 sellTax_) external onlyOwner{
        require(buyTax_ <= 35 && sellTax_<= 90, "Taxes can't exceed the limit.");
