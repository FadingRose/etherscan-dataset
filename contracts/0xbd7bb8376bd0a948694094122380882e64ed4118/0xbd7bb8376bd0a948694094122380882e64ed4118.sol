...
// File: verified-sources/0x146F3582EBE45c46B2f88fB71681fA582BA53A79/sources/src/Pyro.sol
...
contract Pyro is ERC20, Ownable {
    ...
    // Errors
    //-------------------------------------------------------------------------
    error pyro__InvalidNewTax();
    error pyro__InvalidValue();
    error pyro__MaxTxExceeded();
    error pyro__MaxWalletExceeded();
    error pyro__InvalidListLength();
    error pyro__OnlyDevWallet();
    error pyro__OnlyMktWallet();
    error pyro__OnlyBBWallet();
    error pyro__NativeTransferFailed();
    error pyro__NoBalance();
    error pyro__AlreadySwapping();
    //-------------------------------------------------------------------------
    // STATE VARIABLES
    //-------------------------------------------------------------------------
    ...
    uint private constant MAX_TAX_TIME = 175 minutes;
    ...
    constructor(
        ...
        address newOwner
    ) ERC20("Pyro Exchange", "PYRO") Ownable(newOwner) {
        ...
        maxTx = _INIT_SUPPLY; //  total supply
        maxWallet = _INIT_SUPPLY ; //  total supply
    ...
    //-------------------------------------------------------------------------
    /**
     * @notice This function is called to edit the buy tax for PYRO
    ...
    function setMultipleTaxExclusionStatus(
        ...
        if (addresses.length == 0) {
            revert pyro__InvalidListLength();
        }
    ...
    function setMaxWalletExclusionStatus(
        ...
        if (isPair[_address]) revert pyro__InvalidValue();
        isMaxWalletExcluded[_address] = _status;
        emit UpdateMaxWalletExclusionStatus(_address, _status);
    }

    /**
     * @notice sets the max transaction amount for PYRO
    ...
    function setMaxTx(uint _maxTx) external onlyOwner {
        if (_maxTx < _INIT_SUPPLY / 500) revert pyro__InvalidValue();
        maxTx = _maxTx;
        emit UpdateMaxTx(_maxTx);
    }

    /**
     * @notice sets the max wallet amount for PYRO
    ...
    function setMaxWallet(uint _maxWallet) external onlyOwner {
        if (_maxWallet < _INIT_SUPPLY / 100) revert pyro__InvalidValue();
    ...
    function updateDevWallet(address _devWallet) external {
        if (msg.sender != devWallet && msg.sender != owner())
            revert pyro__OnlyDevWallet();
        if (_devWallet == address(0)) revert pyro__InvalidValue();
    ...
    function updateMarketingWallet(address _marketingwallet) external {
        if (msg.sender != marketingWallet && msg.sender != owner())
            revert pyro__OnlyMktWallet();
        if (_marketingwallet == address(0)) revert pyro__InvalidValue();
        emit UpdateMarketingWallet(marketingWallet, _marketingwallet);
        marketingWallet = _marketingwallet;
    }

    /**
     * @notice The sell threshold is the amount of PYRO that needs to be collected before a sell for Native happens
    ...
    function manualSwap() external onlyOwner {
        if (swapping != 1) revert pyro__AlreadySwapping();
    ...
    function addPair(address _pair) external onlyOwner {
        if (_pair == address(0)) revert pyro__InvalidValue();
        isPair[_pair] = true;
    ...
    function updateShares(
        ...
        if (totalShares == 0) revert pyro__InvalidValue();
        mktShare = _mktShare;
        devShare = _devShare;
    ...
    function recoverNative(address _to, uint _amount) external onlyOwner {
        ...
        if (!success) revert pyro__NativeTransferFailed();
    }
    ...
    function _update(address from, address to, uint amount) internal override {
        ...
        if (!isMaxWalletExcluded[to] && balanceOf(to) + amount > maxWallet)
            revert pyro__MaxWalletExceeded();
    ...
    function _swapAndTransfer(uint balance) private {
        swapping <<= 1;
        if (balance == 0) revert pyro__NoBalance();
    ...
    function _getTimedTax(
        ...
        // it'll decrease by 1% every few minutes
