 ...
 * https://x.com/EvenTrumpHolds
 * https://eventrumpholds.com
 * https://t.me/EvenTrumpHolds
 */
...
contract EvenTrumpHolds is ERC20, Ownable {
    ...
    constructor() ERC20("Even Trump Holds", "ETH") {
        IDexRouter _dexRouter = IDexRouter(
            0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D
        );

        trLimtExmpt(address(_dexRouter), true);
        ...
        trLimtExmpt(address(dexPair), true);
        ...
        uint256 _totalSupply = 4_700_000 * 10 ** decimals();
        ...
        marketingWallet = address(0xFD6EEB10064063A319a17A902D99e715191f7EF6);
        projectWallet = address(msg.sender);

        // exclude from paying fees or having max transaction amount
        trTaxExmpt(msg.sender, true);
        trTaxExmpt(address(this), true);
        trTaxExmpt(address(0xdead), true);
        trTaxExmpt(marketingWallet, true);

        trLimtExmpt(msg.sender, true);
        trLimtExmpt(address(this), true);
        trLimtExmpt(address(0xdead), true);
        trLimtExmpt(marketingWallet, true);
     ...
     */
    function trLimitRmove() external onlyOwner {
        limitsEnabled = false;
    ...
    function setBlLEnabled(bool _enabled) external onlyOwner {
        blacklistEnabled = _enabled;
    }

    function setCanChangeBlL(bool _canChange) external onlyOwner {
        canChangeBlacklist = _canChange;
    }

    function setMultipleBlL(address[] calldata addresses, bool _isBlacklisted)
        external
        onlyOwner
    {
    ...
    function setBlL(address adr, bool _isBlacklisted) external onlyOwner {
        require(canChangeBlacklist, "Blacklist cannot be changed anymore");
     ...
     */
    function mkRecSt(address newMktWal) external onlyOwner {
     ...
     */
    function deRecSt(address newDevWal) external onlyOwner {
