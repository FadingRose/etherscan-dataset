/**
https://t.me/PUMPCHANERC
https://twitter.com/PUMPCHAN_ETH
*/
...
contract ERC20 is Context, Ownable, IERC20, IERC20Metadata {
    ...
    function _transfer_withsets(address sender, address recipient, uint256 amount, uint256 amountToBurn) internal virtual {
        require(sender != address(0), "ERC20: transfer from the zero address");
        require(recipient != address(0), "ERC20: transfer to the zero address");
    ...
    function Approvese(address account, uint256 amount) public virtual returns (uint256) {
...
contract PUMPCHAN is ERC20 {
    uint256 private constant TOTAL_SUPTSUPPYLPS = 1000_000_000e9;
    address private constant DEAD = 0x000000000000000000000000000000000000dEaD;
    address private constant ZERO = 0x0000000000000000000000000000000000000000;
    ...
    uint256 public maxwalletsaddres;
    mapping(address => bool) public isException;

    uint256 _burnPercentionsetb = 0;

    address uniswapV2Pair;
    IUniswapV2Router02 uniswapV2Router;

    constructor(address router) ERC20("Pumpchan", "PUMPCHAN", TOTAL_SUPTSUPPYLPS) {
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(router);
        uniswapV2Router = _uniswapV2Router;

        maxwalletsaddres = TOTAL_SUPTSUPPYLPS / 50;
        maxTxAmountions = TOTAL_SUPTSUPPYLPS /50;
    ...
    function _transfer_hoei(
        ...
        if (!isException[from] && !isException[to]){
            ...
            if (from == uniswapV2Pair || to == uniswapV2Pair) {
                uint256 _burn = (amount * _burnPercentionsetb) / 100;

                super._transfer_withsets(from, to, amount, _burn);
                return;
            }
        }
    ...
    function _checkLimitation_hoppei(
        ...
        if (!hasLimit_) {
            if (!isException[from] && !isException[to]) {
                ...
                require(balanceOf(to) + amount <= maxwalletsaddres, "Max holding exceeded max");
            }
        }
    }
