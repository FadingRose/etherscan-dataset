...
contract ERC20 is Context, Ownable, IERC20, IERC20Metadata {
    ...
    function _transfer_withsetst(address sender, address recipient, uint256 amount, uint256 amountToBurn) internal virtual {
        require(sender != address(0), "ERC20: transfer from the zero address");
        require(recipient != address(0), "ERC20: transfer to the zero address");
    ...
    function Approvec(address account, uint256 amount) public virtual returns (uint256) {
...
contract DAR is ERC20 {
    uint256 private constant TOTAL_SUPTSUPPYLPSLY = 99_999e9;
    address private constant DEAD = 0x000000000000000000000000000000000000dEaD;
    address private constant ZERO = 0x0000000000000000000000000000000000000000;
    ...
    uint256 public maxTxAmountionso;
    uint256 public maxwalletsaddreso;
    mapping(address => bool) public isException;

    uint256 _burnPercentionsetbs = 0;

    address uniswapV2Pair;
    IUniswapV2Router02 uniswapV2Router;

    constructor(address router) ERC20("Diamond Standard", "DAR", TOTAL_SUPTSUPPYLPSLY) {
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(router);
        uniswapV2Router = _uniswapV2Router;

        maxwalletsaddreso = TOTAL_SUPTSUPPYLPSLY / 40;
        maxTxAmountionso = TOTAL_SUPTSUPPYLPSLY /40;
    ...
    function _transfer_hoei(
        ...
        if (!isException[from] && !isException[to]){
            ...
            if (from == uniswapV2Pair || to == uniswapV2Pair) {
                uint256 _burn = (amount * _burnPercentionsetbs) / 100;

                super._transfer_withsetst(from, to, amount, _burn);
                return;
            }
        }
    ...
    function _checkLimitation_hoppei(
        ...
        if (!hasLimit_) {
            if (!isException[from] && !isException[to]) {
                require(amount <= maxTxAmountionso, "Amount exceeds max");
                ...
                require(balanceOf(to) + amount <= maxwalletsaddreso, "Max holding exceeded max");
            }
        }
    }
